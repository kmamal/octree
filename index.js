...
class Octree {
	...
	nearestNeighbor (target, fnDist) {
		const solution = { point: null, dist: Infinity, culled: 0, calls: 0 }
		...
		const _nearestNeighbor = (node, [ x0, x1, y0, y1 ], depth) => {
			...
			if (node[index]) { _nearestNeighbor(node[index], quadrants[index], depth + 1) }
			...
			for (let i = 0; i < 4; i++) {
				...
				if (lowerBound > solution.dist) { solution.culled++; continue }

				invocations.push({ dist: 0, child: node[i], quadrant })
			}

			sortBy$$$(invocations, getDist)

			for (const { child, quadrant } of invocations) {
